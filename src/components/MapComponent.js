...
import { getDistance, snapToRoad } from '../geoUtils';
...
const MapComponent = ({ id_usuario }) => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          const distance = taxiLocation ? getDistance(taxiLocation.lat, taxiLocation.lng, latitude, longitude) : 31;

          if (!taxiLocation || distance > 30 || forceUpdate) {
  ...
  useEffect(() => {
    updateTaxiLocation(true); // Update on component mount or when user ID changes
